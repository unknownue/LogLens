...
- **Session restore** — reopens the tabs from the previous run; missing files show a warning instead of crashing. The **window size is remembered too**: it is applied before the window first shows (no visible jump from the default), then saved on resize. Size only — not position, since restoring a position is unreliable across monitor changes. A missing, corrupt, or absurd saved value falls back to the default. Stored as `window-size.json` in the app data dir.
...
| Window size memory | applied in `setup` before first show, saved on resize (debounced, atomic) | `src-tauri/src/window_state.rs` |
